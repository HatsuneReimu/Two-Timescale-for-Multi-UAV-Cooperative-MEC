"""
下限等同于贪心算法最优解 (_get_greedy_solution)：我在 solve 中将贪心代码的核心评估逻辑完全植入，将其得出的最优解作为 0号狼 的起点。即便 GWO 后续全在做无用功，它最终输出的最差结果也等于贪心算法的最优结果。
...
消除群智能算法“假收敛” (positions[i, :] = discrete_sol.astype(float))：原先的代码虽然在评价时做过位置修复，但是更新 Alpha 狼时用的却是修复前的虚假浮点数坐标。现在的代码强制将优化好的、修复后的整数解回写到狼的位置里，真正实现了“在有效解空间”内寻优。
"""
import numpy as np
    ...
    def solve(self, pop_size=30, max_iter=100):
        ...
        for t in range(max_iter):
            for i in range(pop_size):
                ...
                # [核心修正] 将修复优化后的离散位置覆盖回连续位置，防止狼群朝着无效梯度乱跑
