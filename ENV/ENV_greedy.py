...
2. 使用贪心算法 (Greedy Algorithm) 替代 GWO，进行任务调度和计算资源分配。
3. [最新修改] 用户优先把任务上传给距离自己最近的存活无人机，然后再贪心选择哪一家存活无人机为其处理任务。
4. [最新修改] 总时延严格包含：上传时延 + 中转时延（同节点为0） + 计算时延。
"""
...
class ENV:
    ...
    def _greedy_task_allocation(self, failure_status, task_sizes, task_complexities):
        ...
        for m in sorted_users:
            ...
            for u in active_uavs:
                ...
                # 估算中转传输时延 (A2A) t_trans
                t_trans = 0
                if acc_uav != u:  # 如果处理UAV和上传UAV不同，则需要中转
                ...
                # 估算计算时延（假设分配后，该处理UAV的计算资源按sqrt(W)完全分配）
        ...
        for u in active_uavs:
            ...
            for m in users:
                ...
                if acc_uav != -1:
                    r_up = rate_g2a[m, acc_uav]
                    if r_up > 1e-5:
                        t_up = task_sizes[m] / r_up

                # 2. 中转传输时延 t_trans
                t_trans = 0
                if acc_uav != -1 and acc_uav != u:
                    r_tr = rate_a2a[acc_uav, u]
                    if r_tr > 1e-5:
                        t_trans = task_sizes[m] / r_tr
                    else:
        ...
        # 对于未成功连接任何UAV进行处理的用户，施加极大时延惩罚
        for m in range(self.num_users):
            if solution[m] == -1:
                user_latencies[m] = 1e6
    ...
    def step(self, actions):
        ...
        # 更新 self.user_uav_connections 以指向距离最近的存活无人机
        ...
        for _ in range(1):
            ...
            task_complexities = np.random.uniform(self.task_complexity_range[0], self.task_complexity_range[1], self.num_users)
        ...
        """
        print("UAV positions:\n", self.uav_positions)
        print("Global Delay Reward:", global_delay_reward)
        print("Energy Penalties:", energy_penalties)
