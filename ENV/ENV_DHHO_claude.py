...
class DHHOSolver:  # 已替换为 DHHO 内核，类名保持兼容
    ...
    def _get_greedy_solution(self):
        """
        [新增核心] 贪心初始化策略：
        利用贪心思想生成一个极高质量的初始解，确保GWO的下限必定高于纯贪心算法。
    ...
    def _clip_to_uav(self, pos):
        """将连续位置裁剪到合法 UAV 索引范围 [0, num_uavs-1]。"""
        return np.clip(pos, 0, self.env.num_uavs - 1)

    # ------------------------------------------------------------------
    # DHHO 主求解器（替换原 GWO solve）
    # ------------------------------------------------------------------
    def solve(self, pop_size=30, max_iter=100):
        """
        DHHO（Dynamic Harris Hawks Optimization，动态哈里斯鹰优化）求解器。

        核心改进（相比 GWO）：
        1. 动态逃逸能量 E：随迭代非线性衰减，前期全局探索更充分。
        2. 四种攻击策略根据 |E| 自适应切换：
           - 软包围（|E|≥0.5）：向猎物缓慢逼近。
           - 硬包围（|E|<0.5 且 J≥0.5）：快速直接逼近。
           - 软包围 + Lévy 飞行（|E|≥0.5 且 J<0.5）：大范围跳跃探索。
           - 硬包围 + Lévy 飞行（|E|<0.5 且 J<0.5）：精细局部开采。
        3. 贪心种子注入：兔子（猎物）初始化为高质量贪心解，同 GWO 保底策略。
        4. 离散解回写：修复后的整数解强制覆盖连续位置，避免在无效空间寻优。
        """
        if len(self.active_uavs) == 0:
            return None, []
        ...
        # ---------- 2. 初始化猎物（Rabbit）为贪心解 ----------
        rabbit_pos = greedy_seed.astype(float).copy()
        ...
        for t in range(max_iter):
            ...
            # 3-b. 计算动态逃逸能量 E
            #   E0 ∈ [-1, 1] 随机，|E| 随迭代从 2→0 非线性衰减
            E0 = 2 * random.random() - 1
            # 动态因子：前期衰减慢（全局探索），后期衰减快（局部开采）
            decay = 1 - (t / max_iter) ** 2          # 非线性衰减，比线性更平滑
            E = 2 * E0 * decay                        # E ∈ [-2, 2] → 逐渐趋近 0
            ...
            for i in range(pop_size):
                ...
                if abs_E >= 1:
                    # ---- 探索阶段：随机栖息 ----
                    # 随机选一只其他鹰作为参考点，避免原地踏步
                    rand_hawk_idx = random.randint(0, pop_size - 1)
                    X_rand = positions[rand_hawk_idx, :].copy()
                    r = random.random()
                    # 随机跳到某个合法 UAV，再向随机鹰方向漂移
                    rand_uav = np.array([
                        float(random.choice(self.active_uavs)) for _ in range(dim)
                    ])
                    new_X = X_rand - r * np.abs(X_rand - 2 * r * rand_uav)

                else:
                    # ---- 开采阶段：四种攻击策略 ----
                    J = 2 * (1 - random.random())    # 兔子随机跳跃幅度
                    r1 = random.random()

                    if r1 >= 0.5 and abs_E >= 0.5:
                        # 策略1：软包围
                        delta_X = rabbit_pos - X
                        new_X = delta_X - E * np.abs(J * rabbit_pos - X)

                    elif r1 >= 0.5 and abs_E < 0.5:
                        # 策略2：硬包围
                        new_X = rabbit_pos - E * np.abs(rabbit_pos - X)

                    elif r1 < 0.5 and abs_E >= 0.5:
                        # 策略3：软包围 + Lévy 飞行（大范围跳跃）
                        LF = self._levy(dim)
                        Y = rabbit_pos - E * np.abs(J * rabbit_pos - X)
                        Y = self._clip_to_uav(Y)
                        fit_Y, _, dsol_Y, _, _ = self.calculate_objective(Y)
                        Z = Y + LF * np.random.uniform(-0.5, 0.5, dim)
                        Z = self._clip_to_uav(Z)
                        fit_Z, _, dsol_Z, _, _ = self.calculate_objective(Z)
                        if fit_Y < fit_Z:
                            new_X = dsol_Y.astype(float)
                        else:
                            new_X = dsol_Z.astype(float)

                    else:
                        # 策略4：硬包围 + Lévy 飞行（精细局部开采）
                        LF = self._levy(dim)
                        # 计算种群均值作为参考
                ...
                positions[i, :] = self._clip_to_uav(new_X)
