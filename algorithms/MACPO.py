...
from CAMRL.algorithms.actor_critic import R_Actor
...
class R_MACTRPO_CPO():
    ...
    def __init__(self, args, obs_space, cent_obs_space, device, act_space,attempt_feasible_recovery=False,
                 attempt_infeasible_recovery=False, revert_to_last_safe_point=False,_backtrack_ratio=0.8, _max_backtracks=15,linesearch_infeasible_recovery=True, accept_violation=False):
    ...
    def kl_divergence(self, obs, action, new_actor, old_actor, available_actions=None, active_masks=None):

        _, _, mu, std = new_actor.evaluate_actions(obs, action,available_actions,active_masks)
        _, _, mu_old, std_old = old_actor.evaluate_actions(obs,action)
