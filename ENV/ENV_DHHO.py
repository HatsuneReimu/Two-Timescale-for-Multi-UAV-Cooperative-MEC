...
class DHHOSolver:
    ...
    def heuristic_repair(self, uav_indices):
        ...
        uav_indices = uav_indices.copy()  # 避免原地修改导致状态污染
        for _ in range(2):
    ...
    def calculate_objective(self, positions):
        """计算适应度 (支持直接传入离散整数数组)"""
        uav_indices = np.array(positions).astype(int)
    ...
    # ------------------------------------------------------------------
    # 真·离散 DHHO 求解器 (True Discrete HHO)
    # ------------------------------------------------------------------
    def solve(self, pop_size=30, max_iter=50):
        """
        高度优化的离散哈里斯鹰优化算法 (Discrete HHO)
        完全摒弃连续相加的物理无意义操作，转而使用：
        - 概率型基因交叉 (Discrete Crossover)
        - 重尾分布的突变映射 (Levy-based Mutation)
        适配组合优化问题，性能比暴力连续转换法提升几个数量级。
        """
        if len(self.active_uavs) == 0:
            return None, []

        dim = self.env.num_users
        positions = np.zeros((pop_size, dim), dtype=int)
        ...
        for i in range(pop_size):
            if i == 0:
                ...
                # 在贪心解基础上进行 20% 变异
                pos = greedy_seed.copy()
                mask = np.random.rand(dim) < 0.2
                pos[mask] = np.random.choice(self.active_uavs, size=np.sum(mask))
                positions[i, :] = pos
            else:
                # 完全随机生成
                positions[i, :] = np.random.choice(self.active_uavs, size=dim)

        rabbit_pos = greedy_seed.copy()
        rabbit_score = float("inf")
        best_details = None
        convergence_curve = []

        # 记录每只鹰的当前适应度，便于在 Rapid Dive 中对比
        fitness_array = np.full(pop_size, float("inf"))
        ...
        for t in range(max_iter):
            ...
            for i in range(pop_size):
                ...
                positions[i, :] = discrete_sol  # 写入修复后的合法解
                fitness_array[i] = fit

                if fit < rabbit_score:
                    rabbit_score = fit
                    rabbit_pos = discrete_sol.copy()
            ...
            for i in range(pop_size):
                ...
                # 防止种群过早陷入停滞：如果和兔子一样，强制重置部分基因
                if np.array_equal(X, rabbit_pos) and i != 0:
                    mask = np.random.rand(dim) < 0.5
                    X[mask] = np.random.choice(self.active_uavs, size=np.sum(mask))
                    positions[i, :] = X
                    continue
                ...
                if abs_E >= 1:
                    ...
                    if q >= 0.5:
                        ...
                        mask = np.random.rand(dim) < 0.5
                        X[mask] = positions[rand_idx, mask]
                    else:
                        # 策略2：大范围随机探索 (离散变异)
                        mask = np.random.rand(dim) < 0.5
                        X[mask] = np.random.choice(self.active_uavs, size=np.sum(mask))
                    ...
                    if r >= 0.5 and abs_E >= 0.5:
                        # 策略1：软包围 (以 1-|E| 的概率向兔子基因靠拢)
                        prob_copy = 1.0 - abs_E  # abs_E 越大，继承越少
                        mask = np.random.rand(dim) < prob_copy
                        X[mask] = rabbit_pos[mask]

                    elif r >= 0.5 and abs_E < 0.5:
                        # 策略2：硬包围 (以较高概率继承兔子基因)
                        prob_copy = 1.0 - (abs_E / 2.0)
                        mask = np.random.rand(dim) < prob_copy
                        X[mask] = rabbit_pos[mask]

                    elif r < 0.5 and abs_E >= 0.5:
                        # 策略3：软包围 + 快速俯冲 (带有局部搜索的试探)
                        # 生成试探解 Y (兔子 + 轻微变异)
                        Y = rabbit_pos.copy()
                        mask_y = np.random.rand(dim) < 0.1
                        Y[mask_y] = np.random.choice(self.active_uavs, size=np.sum(mask_y))
                        fit_Y, _, dsol_Y, _, _ = self.calculate_objective(Y)

                        if fit_Y < fitness_array[i]:
                            X = dsol_Y
                        else:
                            # 试探解失败，使用 Levy 突变生成 Z (大尺度变异)
                            Z = Y.copy()
                            levy_step = abs(self._levy(1)[0])
                            mut_prob = np.clip(0.1 * levy_step, 0.1, 0.6)  # Levy映射为变异率
                            mask_z = np.random.rand(dim) < mut_prob
                            Z[mask_z] = np.random.choice(self.active_uavs, size=np.sum(mask_z))

                            fit_Z, _, dsol_Z, _, _ = self.calculate_objective(Z)
                            if fit_Z < fitness_array[i]:
                                X = dsol_Z

                    else:
                        # 策略4：硬包围 + 快速俯冲 (深度开采与突变)
                        # 生成试探解 Y (完全克隆兔子 + 极微小变异)
                        Y = rabbit_pos.copy()
                        mask_y = np.random.rand(dim) < 0.05
                        Y[mask_y] = np.random.choice(self.active_uavs, size=np.sum(mask_y))
                        fit_Y, _, dsol_Y, _, _ = self.calculate_objective(Y)
                        ...
                        if fit_Y < fitness_array[i]:
                            ...
                            # 试探解失败，使用 Levy 突变生成 Z
                            Z = Y.copy()
                            levy_step = abs(self._levy(1)[0])
                            mut_prob = np.clip(0.05 * levy_step, 0.05, 0.4)
                            mask_z = np.random.rand(dim) < mut_prob
                            Z[mask_z] = np.random.choice(self.active_uavs, size=np.sum(mask_z))
...
class ENV:
    def __init__(self, args):
        ...
        self.energy_penalty_coeff = 0.02
    ...
    def step(self, actions):
        ...
        for i in range(self.num_uavs):
            final_rewards[i] = (0.5 * delay_reward[i]) + (0.5 * energy_penalties[i]) + boundary_penalties[i] + speed_penalties[i] + collision_penalties[i]
            final_costs[i] = boundary_penalties[i] + speed_penalties[i] + collision_penalties[i]
            final_delays[i] = 0.5 * delay_reward[i]
            final_energys[i] = 0.5 * energy_penalties[i]
